...
def cal_portfolio(tickers,adj_price_data, output_file_path):
    panel_data = adj_price_data
    ...
    annual_returns = combined_returns.groupby('Year').agg({'Saif\'s Fund': 'sum', 'GSPC Monthly Return': 'sum'})

    plt.figure(figsize=(10, 6))

    # Bar chart for annual returns
    bar_width = 0.35
    plt.bar(annual_returns.index - bar_width/2, annual_returns["Saif's Fund"], bar_width, label="Saif's Fund")
    ...
    plt.step(cumulative_returns.index, cumulative_returns["Cumulative Saif's Fund"], label="Cumulative Saif's Fund", where='post')
